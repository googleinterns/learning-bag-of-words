...
class AsrEncoder(base_layer.BaseLayer):
  ...
  def __init__(self, params):
    ...
    for i in range(p.num_lstm_layers):
      ...
      # rnn_p.sequence_length = math.ceil(math.ceil(p.num_frames / 2. + 1) / 2. + 1)
  ...
  def FProp(self, theta, batch, state0=None):
    ...
    with tf.name_scope(p.name):
      ...
      for i, (rnn, cnn) in enumerate(zip(self.conv_lstm_rnn, self.conv_lstm_cnn)):
        ...
        conv_lstm_in_padding = tf.expand_dims(tf.transpose(conv_lstm_out_padding), 2)
        lstm_out = rnn.FProp(theta.conv_lstm_rnn[i], conv_lstm_in, conv_lstm_in_padding)
      ...
      # outputs['encoded'] = tf.ensure_shape(outputs['encoded'], [10, 1, 5])
      # outputs['padding'] = tf.ensure_shape(outputs['padding'], [10, 1])
