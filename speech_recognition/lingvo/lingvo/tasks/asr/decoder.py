...
class AsrDecoderBase(base_decoder.BaseBeamSearchDecoder):
  ...
  def Params(cls):
    ...
    p.Define('empty_attention_context', False, 'feed an empty attention at each step')
  ...
  def BaseZeroState(self,
    ...
    if packed_src is None:
      ...
      zero_atten_state = self.atten.ZeroAttentionState(None, bs)
  ...
  def ComputeLoss(self, theta, predictions, targets):
    ...
    with tf.name_scope(p.name):
      ...
      elif targets.one_hot_labels is not None:
        target_probs = targets.one_hot_labels
      else:
        target_probs = None
      merged_metrics = {}
  ...
  def ComputePredictions(self, theta, encoder_outputs, targets, attention_contexts=None):
    """Computes logits.
    ...
    if p.use_while_loop_based_unrolling:
      predictions = self.ComputePredictionsDynamic(theta, encoder_outputs, targets, attention_contexts, targets.softmax_input)
    else:
  ...
  def _GetNewAttenProbs(self, seq_out_tas, time, decoder_step_state):
    ...
    if decoder_step_state.atten_probs is None:
      return seq_out_tas.atten_probs
  ...
  def _UpdateSequenceOutTensorArrays(self, decoder_step_state, time, step_outs,
    ...
    if decoder_step_state.rnn_states is not None:
    ...
    if decoder_step_state.misc_states is not None:
  ...
  def _GetPredictionFromSequenceOutTensorArrays(self, seq_out_tas):
    ...
    self.logits = tf.transpose(seq_out_tas.logits.stack(), [1, 0, 2])
    if self.params.confidence is not None:
  ...
  def _GetInitialTargetInfo(self, targets, max_seq_length, target_embs):
    return AsrDecoderBase.TargetInfo(
        id=None if targets.ids is None else _ToTensorArray(
            'target_ids_ta',
            tf.transpose(targets.ids),
            max_seq_length,
            clear_after_read=False),
        label=None if targets.ids is None else _ToTensorArray(
            'target_labels_ta',
  ...
  def ComputePredictionsDynamic(self, theta, encoder_outputs, targets, attention_contexts=None, softmax_input=None):
    p = self.params

    self.atten_context = None
    ...
    with tf.name_scope(p.name):
      ...
      # training.
      # dec_bs = 2
      dec_bs = targets.batch_size
      max_seq_length = tf.shape(targets.paddings)[1]

      if targets.ids is not None:
        target_embs = self.emb.EmbLookup(theta.emb, tf.reshape(targets.ids, [-1]))
        if self.att_context is None:
          self.att_context = targets.ids
      elif False: # True:
        weights, ids = tf.math.top_k(targets.one_hot_ids, k=20)  # 1 x l x k
        target_embs = self.emb.EmbLookup(theta.emb, ids)  # 1 x l x k x V
        if self.att_context is None:
          self.att_context = ids
        target_embs = target_embs * tf.expand_dims(tf.nn.relu(weights), -1)  # 1 x l x k x V
        target_embs = tf.reduce_sum(target_embs, -2)  # 1 x l x V
        target_embs = tf.reshape(target_embs, [1, max_seq_length, -1])
      else:
        target_embs = 0
        ri = p.emb.vocab_size - p.emb.vocab_size // len(theta.emb['wm']) * len(theta.emb['wm'])
        cur_l = 0
        for i in range(len(theta.emb['wm'])):
          l = p.emb.vocab_size // len(theta.emb['wm'])
          if i < ri:
            l += 1
          target_embs = target_embs + \
                        tf.linalg.matmul(targets.one_hot_ids[:, :, cur_l:cur_l + l], theta.emb['wm'][i][:l, :])
          cur_l += l
      ...
      decoder_step_state_zero, packed_src = self.DecoderStepZeroState(
          theta, encoder_outputs, tf.zeros([1, dec_bs], dtype=tf.float32), dec_bs)
      if attention_contexts is not None:
        decoder_step_state_zero.atten_context = attention_contexts[:, 0]
      else:
        all_attention_contexts.append(decoder_step_state_zero.atten_context)
      decoder_step_state_zero_fusion_flat = (
      ...
      def _LoopBody(time, old_decoder_step_state, target_info_tas, seq_out_tas, last=False):
        """Computes decoder outputs and updates decoder_step_state."""
        cur_target_info = self.TargetsToBeFedAtCurrentDecodeStep(
            time, theta, old_decoder_step_state, target_info_tas, seq_out_tas)

        attention_context = None if attention_contexts is None else attention_contexts[:, time + 1 if not last else 0]
        step_out = None if softmax_input is None else softmax_input[time]

        step_outs, decoder_step_state = self.SingleDecodeStep(
            theta, packed_src, cur_target_info, old_decoder_step_state, attention_context, step_out)

        all_step_outs.append(step_outs)
        all_attention_contexts.append(decoder_step_state.atten_context)
        ...
        xent_loss = self.softmax.FProp(
            theta.softmax, [step_outs] if softmax_input is None else [softmax_input[time]],
            class_weights=targets.weights[:, time],
            # class_ids=cur_target_info.label,
            class_probabilities=targets.one_hot_labels[:, time])
      ...
      decoder_step_state = decoder_step_state_zero
      for i in range(p.target_seq_len):
        time, decoder_step_state, target_info_tas, seq_out_tas = _LoopBody(
          time, decoder_step_state, target_info_tas, seq_out_tas, last=i==p.target_seq_len - 1)
      ...
      self.atten_context = tf.reshape(tf.stack(all_attention_contexts[:-1]), [1, -1, 2048]) if len(all_attention_contexts) > 0 else None
      self.softmax_input = tf.stack(all_step_outs)
    ...
    with tf.name_scope(p.name):
      ...
      if p.confidence is not None:
        ...
        confidence_logits = tf.squeeze(
            self.confidence.FProp(theta.confidence, tf.stop_gradient(confidence_input)),
            axis=-1)
  ...
  def TargetsToBeFedAtCurrentDecodeStep(self, time, theta, decoder_step_state,
    ...
    if target_info_tas.id is not None:
...
class AsrDecoder(AsrDecoderBase):
  ...
  def SingleDecodeStep(self,
    ...
    elif self.params.empty_attention_context:
      new_atten_context = tf.zeros([1, 2048])
      new_atten_probs = None
      new_atten_states = None
    elif atten_context is not None:
      new_atten_context = atten_context
      new_atten_probs = None
      new_atten_states = None
    else:
      new_atten_context, new_atten_probs, new_atten_states = self._ComputeAttention(
        theta,
        rnn_out,
        packed_src,
